        ...
        try {
            ...
            if (phpInterpreter == null || phpInterpreter.pathToPhpExecutable == null) {
                Notifications.Bus.notify(
                    NotificationGroup.Companion.balloonGroup("PHP Interpreter error")
                        .createNotification(
                            "PHP Interpreter error",
            ...
            ProgressManager.getInstance().run(object : Backgroundable(project, "Running Tinker") {
                override fun run(progressIndicator: ProgressIndicator) {
                    ...
                    if (!processHandler.waitFor()) {
                        Notifications.Bus.notify(
                            NotificationGroup.Companion.balloonGroup("Could not wait for process finish")
                                .createNotification(
            ...
            Notifications.Bus.notify(
                NotificationGroup.balloonGroup("Could not run tinker")
                    .createNotification(
                        "Could not run tinker",
                        NotificationType.ERROR
                    ),
                project
            )
