...
class MakeSureConsolesHaveExtensions {
    ...
    fun up() {
        val consoleDirectory = ScratchFileService.getInstance().getRootPath(LaravelTinkerConsolesRootType.getInstance())
        File(consoleDirectory).listFiles()?.forEachIndexed { i, file ->
            file.renameTo(
